...
In your model, you can specify a style for the :original image and also turn off
the reprocessing of the :original image. This allows you to reduce the size
of a gigantic image **on the upload**, but then leave the :original image untouched
every time you do further re-cropping.

  class User < ActiveRecord::Base
    has_attached_file :avatar,
                      :styles => { :medium => "300x300>", :thumb => "100x100>", :original => "750" },
                      :reprocess_original => false,
                      :processors => [:cropper]
  end
